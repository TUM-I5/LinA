...
db = parseJSONMatrixFile('{}/matrices_{}.json'.format(cmdLineArgs.matricesDir, degree), transpose=False)
...
## Main kernels
volume = (Q['xyp'] <= Q['xyp'] + db.kDivM['xl'] * I['lyq'] * db.star[0]['qp'] + db.kDivM['ym'] * I['xmq'] * db.star[1]['qp'])
g.add('volume', volume)

def flux(dim,side1,side2):
  if dim == 0:
    return Q['xyp'] <= Q['xyp'] + db.FDivM[side1,side2]['xl'] * I['lyq'] * fluxSolver['qp']
  return Q['xyp'] <= Q['xyp'] + db.FDivM[side1,side2]['ym'] * I['xmq'] * fluxSolver['qp']
...
for i in range(1,order):
  derivativeSum = db.kDivMT['xl'] * derivatives[-1]['lyq'] * db.star[0]['qp'] + db.kDivMT['ym'] * derivatives[-1]['xmq'] * db.star[1]['qp']
